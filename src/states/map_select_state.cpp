...
void MapSelectState::render(Game& game) {
    ...
    for (int i = 0; i < static_cast<int>(maps.size()); ++i) {
        ...
        sel_text(a, maps[i].c_str(), SCREEN_WIDTH / 2.0f - 100, static_cast<float>(y + 15), 24, sel ? WHITE : LIGHTGRAY);
        if (i < 3) {
            sel_text(a, descriptions[i], SCREEN_WIDTH / 2.0f - 100, static_cast<float>(y + 45), 14, sel ? LIGHTGRAY : GRAY);
        }
    }
    ...
    sel_text(a, std::format("Difficulty: {}", diff_names[di]).c_str(), SCREEN_WIDTH / 2.0f - 80, static_cast<float>(dy + 8), 20, diff_colors[di]);
    ...
    sel_text(a, "Press ENTER to start  |  ESC to go back", SCREEN_WIDTH / 2.0f - 160, static_cast<float>(SCREEN_HEIGHT - 50), 16, GRAY);
}
